...
double _audio_output_get_darwin_version() {
    
    struct utsname name;
    uname(&name);
    
    return strtod(name.release, NULL);
...
struct audio_output_t* audio_output_create(struct decoder_output_format_t decoder_output_format) {
    ...
    double darwin_version = _audio_output_get_darwin_version();
#if TARGET_OS_IPHONE
    double varispeed_darwin_version = 11;
#else
    double varispeed_darwin_version = 7;
#endif
    
    if (darwin_version >= varispeed_darwin_version) { // Darwin 11 is iOS 5. Varispeed is only available in iOS 5+.
